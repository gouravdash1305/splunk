...
_ACCEPTED_CLUSTER_MODES = [_CLUSTER_MODE_DISABLED, _CLUSTER_MODE_SEARCHHEAD]
_ACCEPTED_SERVER_ROLES = [_ROLE_SEARCH_HEAD, _ROLE_KV_STORE, _ROLE_SHC_DEPLOYER]
...
def load_proxy_settings(log: logging.Logger) -> Dict:
    ...
    log.info("Splunk proxy settings, config=%s", proxy_settings)
...
def is_search_head(log: logging.Logger, session_key: str) -> bool:
    ...
    if cluster_mode == _CLUSTER_MODE_SEARCHHEAD:
        return True

    if cluster_mode == _CLUSTER_MODE_DISABLED:
        excluded_role = _ROLE_SEARCH_PEER in server_roles
        if excluded_role:
            return False
...
def get_server_roles(log: logging.Logger, session_key) -> List[str]:
    ...
    log.info("Fetched server roles, roles=%s", roles)
    return roles
...
def get_cluster_mode(log: logging.Logger, session_key):
    ...
    log.info("Fetched cluster mode, mode=%s", mode)
    return mode
...
def get_telemetry_sud_optin(log: logging.Logger, session_key: str):
    ...
    log.info("Telemetry SUD opt-in status fetched, status=%s", sud_value)
