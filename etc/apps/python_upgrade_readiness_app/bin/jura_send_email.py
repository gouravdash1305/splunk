...
logging = logger_manager.setup_logging('jura_send_email')
...
def send_email(email_configurations, user_credentials, session_key, receiver_list, email_body, email_subject=JQUERY_SUBJECT, email_body_type="plain", is_email_to_be_bcced=False):
    """
    Send the email.
...
def send_email_wrapper(session_key, email_body_details):
    ...
    report_file = os.path.join(JQUERY_MERGED_DIR, MERGED_FILE_JSON)
    ...
    dismiss_app_details = utils.get_dismiss_app_kvstore_details(session_key, "jquery")
    dismiss_file_details = utils.get_dismiss_file_kvstore_details(session_key, "jquery")
    ...
    utils.filter_dismissed_files(report, dismiss_file_details, scan_type="jquery")
    ...
    if failed_apps == 1:
        email_body_text = BODY.format(failed_apps, "app", "jQuery", stack_name)
    else:
        email_body_text = BODY.format(failed_apps, "apps", "jQuery", stack_name)
