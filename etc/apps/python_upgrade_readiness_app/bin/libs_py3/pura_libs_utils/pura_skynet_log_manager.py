...
def setup_logging(log_name, log_level=logging.INFO):
    ...
    skynet_log_dir = make_splunkhome_path(["var", "log", "python_upgrade_readiness_app", "skynet"])
    if not os.path.exists(skynet_log_dir):
        os.makedirs(skynet_log_dir)

    log_file = make_splunkhome_path(["var", "log", "python_upgrade_readiness_app", "skynet", "%s.log" % log_name])
