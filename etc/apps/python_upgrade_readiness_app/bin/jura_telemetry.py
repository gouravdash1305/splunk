...
logging = logger_manager.setup_logging('jura_telemetry')
...
class Telemetry(object):
    ...
    def init_telemetry(self):
        ...
        TELEMETRY_DATA = {
            'type': "event",
            'component': "app.jQueryupgradereadiness.scan",
            'optInRequired': 2,
            'data': {
                'appVersion': "4.0.0",
    ...
    def update_telemetry_data(self, report, result, app, app_meta=None, skip_flag=False, default=False):
        ...
        try:
            ...
            for check in report['checks']:
                if check['name'] == "Splunk dashboard jQuery version check":
                    meta_data['JQueryDashboardStatus'] = check['result']
                    meta_data['JQueryDashboardNumber'] = len(check['messages'])
                elif check['name'] == "Splunk jQuery version check":
                    meta_data['JQueryVersionStatus'] = check['result']
                    meta_data['JQueryVersionNumber'] = len(check['messages'])
                elif check['name'] == "Splunk internal library check":
                    meta_data['InternalLibraryStatus'] = check['result']
                    meta_data['InternalLibraryNumber'] = len(check['messages'])
                elif check['name'] == "Splunk hotlinking web library check":
                    meta_data['HotlinkingLibraryStatus'] = check['result']
                    meta_data['HotlinkingLibraryNumber'] = len(check['messages'])
                else:
                    meta_data['DeprecatedHTMLDashboardsStatus'] = check['result']
                    meta_data['DeprecatedHTMLDashboardsNumber'] = len(check['messages'])
    ...
    def add_logs_for_skynet(self, data):
        ...
        """
        try:
            logging.info("Adding jQuery scan summary to logs.")
    ...
    def write_scan_summary(self):
        ...
        try:
            ...
            summary = {}
            summary['appJqueryUpradeReadinessScanDetails'] = {}
            summary['appJqueryUpradeReadinessScanDetails']['statistics'] = self.scan_summary['statistics']
            summary['appJqueryUpradeReadinessScanDetails']['apps'] = self.scan_summary['apps']
            summary['appJqueryUpradeReadinessScanDetails']['summary'] = self.scan_summary['summary']
            summary['appJqueryUpradeReadinessScanDetails']['errors'] = self.scan_summary['errors']
            summary['appJqueryUpradeReadinessScanDetails']['timestamp'] = int(time.time())
            ...
            file_data["appJqueryUpradeReadinessScanDetails"] = summary["appJqueryUpradeReadinessScanDetails"]
    ...
    def write_to_audit(self):
        ...
        try:
            ...
            summary = {}
            summary['appVersion'] = '4.0.0'
            summary['appJqueryUpradeReadinessScanDetails'] = {}
            summary['appJqueryUpradeReadinessScanDetails']['apps'] = self.scan_summary['apps']
            summary['appJqueryUpradeReadinessScanDetails']['summary'] = self.scan_summary['summary']
            summary['appJqueryUpradeReadinessScanDetails']['errors'] = self.scan_summary['errors']
            summary['timestamp'] = int(time.time())
            python_toggle_utils.splunk_push(self.session_key, summary, "jQuery_upgrade_readiness_app")
