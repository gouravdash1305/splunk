...
def send_email(email_configurations, user_credentials, session_key, receiver_list, email_body, email_subject=SUBJECT, email_body_type="plain", is_email_to_be_bcced=False):
    """
    Send the email.
...
def send_email_wrapper(session_key, email_body_details):
    ...
    report_file = os.path.join(MERGED_DIR, MERGED_FILE_JSON)
    ...
    dismiss_app_details = utils.get_dismiss_app_kvstore_details(session_key, "python")
    dismiss_file_details = utils.get_dismiss_file_kvstore_details(session_key, "python")
    if dismiss_app_details is None or dismiss_file_details is None:
        logging.error("all_dismissed_apps or all_dismissed_files is None so returning")
        return
    # filter dismissed apps
    # filter dismissed files
    utils.filter_dismissed_apps(report, dismiss_app_details)
    utils.filter_dismissed_files(report, dismiss_file_details, scan_type="python")
    failed_apps = get_failed_apps_count(report=report)
    ...
    if failed_apps == 1:
        email_body_text = BODY.format(failed_apps, "app", "Python", stack_name)
    else:
        email_body_text = BODY.format(failed_apps, "apps", "Python", stack_name)
    ...
    is_email_sent = send_email(
        ...
        user_credentials=user_credentials, session_key=session_key,
        receiver_list=receiver_list, email_body=email_body_text,
