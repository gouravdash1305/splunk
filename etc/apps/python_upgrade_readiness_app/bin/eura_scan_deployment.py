...
class ScanDeploymentHandler(PersistentServerConnectionApplication):
    ...
    def handle(self, in_string):
        ...
        try:
            ...
            if is_cloud:
                return utils.render_error_json(MESSAGE_CANNOT_START_SCAN_CLOUD, 403)
            scan_type = request_body.get('scanType', 'deployment')
            system_scan = False
            if (scan_type == "deployment") or (scan_type == "config"):
                system_scan = True
            if request_body.get('apps') is None:
                scan_type = request_body.get('scanType', 'deployment')
            ...
            request_body["system_scan"] = system_scan
            if not os.path.exists(LOCAL_DIR):
                os.mkdir(LOCAL_DIR)
            pura_entry_lock = FileLock(
                os.path.join(os.path.dirname(__file__), "..", "local", "eura_entry.lock")
            )
            ...
            with pura_entry_lock.acquire(timeout=120, poll_intervall=5):
                ...
                try:
                    ...
                    exception_msg = "Exception occured while checking whether the instance "\
                                    "is cloud or not: {}"
                    logging.exception(exception_msg.format(str(e)))
                arg_vars = dict()
                ...
                arg_vars["is_cloud"] = is_cloud

                DEVNULL_PATH = open(os.devnull, 'wb')
                if six.PY2:
                    command = "\"{}\" cmd python \"{}\"".format(SPLUNK_PATH, EMERALD_PROCESS_PATH)
                elif six.PY3:
                    command = "\"{}\" cmd python3 \"{}\"".format(SPLUNK_PATH, EMERALD_PROCESS_PATH)
                if os.name == "nt":
